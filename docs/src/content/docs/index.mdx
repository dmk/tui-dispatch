---
title: tui-dispatch
description: Centralized state management for Rust TUI apps
template: splash
hero:
  tagline: A centralized state management framework for Rust TUI applications, inspired by Redux and The Elm Architecture.
  actions:
    - text: Get Started
      link: /tui-dispatch/getting-started/quick-start/
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/dmk/tui-dispatch
      icon: external
---

import { Card, CardGrid } from '@astrojs/starlight/components';

<CardGrid>
  <Card title="Quick Start" icon="rocket">
    Get a running app in minutes with runtime helpers and sensible defaults.

    [Start building →](/tui-dispatch/getting-started/quick-start/)
  </Card>
  <Card title="Core Concepts" icon="open-book">
    Learn the terminology: Actions, State, Reducers, Effects, and more.

    [Read the glossary →](/tui-dispatch/getting-started/core-concepts/)
  </Card>
  <Card title="Async Patterns" icon="puzzle">
    Handle API calls, file I/O, and timers with effects and TaskManager.

    [Learn async →](/tui-dispatch/patterns/async/)
  </Card>
  <Card title="Examples" icon="document">
    From a simple counter to full apps with debug overlays and feature flags.

    [Browse examples →](/tui-dispatch/examples/)
  </Card>
</CardGrid>

## When to Use tui-dispatch

**Good fit:**
- Apps with shared state across multiple UI components
- Apps with async operations (API calls, file I/O)
- Apps where you want clear separation between UI and logic
- Apps that need debugging tools (action logging, state inspection)

**Overkill for:**
- Simple single-screen apps with minimal state
- Apps where state is naturally local to each widget

## How It Works

A reducer takes state and an action, then returns a `ReducerResult`:

```rust
fn reducer(state: &mut AppState, action: AppAction) -> ReducerResult {
    match action {
        AppAction::Increment => { state.count += 1; ReducerResult::changed() }
        AppAction::Quit => ReducerResult::unchanged(),
    }
}
```

Use `Runtime` or `Store` directly. See the [Counter example](/tui-dispatch/examples/counter/).

## Adding Effects

When you need async operations, use the same reducer shape with an effect type:

```rust
fn reducer(state: &mut AppState, action: AppAction) -> ReducerResult<Effect> {
    match action {
        AppAction::Fetch => {
            state.is_loading = true;
            ReducerResult::changed_with(Effect::FetchData)
        }
        AppAction::DidLoad(data) => {
            state.data = Some(data);
            state.is_loading = false;
            ReducerResult::changed()
        }
    }
}
```

Effects are data describing side effects. The main loop executes them outside the reducer with `Runtime::run_with_effects(...)`.

## Data Flow

```
Terminal Input
      │
      ▼
 EventBus.handle_event(event) ──► Actions
                        │
                        ▼
                 store.dispatch(action)
                        │
                        ▼
                 reducer(state, action)
                        │
                        ▼
            ┌───────────┴───────────┐
            │                       │
            ▼                       ▼
     state changed?          effects to run?
            │                       │
            ▼                       ▼
        render()              handle_effect()
                                    │
                                    ▼
                            spawn async task
                                    │
                                    ▼
                         action_tx.send(DidAction)
                                    │
                                    └──► back to dispatch
```

## Crate Structure

```
tui-dispatch/
├── tui-dispatch/        # Re-exports + prelude
├── tui-dispatch-core/   # Store, Runtime, Component, Keybindings, Middleware
├── tui-dispatch-macros/ # #[derive(Action)], #[derive(DebugState)], #[derive(BindingContext)]
└── tui-dispatch-debug/  # Debug overlay, action recording, replay, schema generation
```

Optional companion crate:
```
tui-dispatch-components/ # Reusable UI components: SelectList, TextInput, etc.
```

The core crate also includes [Keybindings](/tui-dispatch/patterns/keybindings/) for context-aware key mapping, [Middleware](/tui-dispatch/patterns/middleware/) for action interception, and [reducer_compose!](/tui-dispatch/patterns/reducer-composition/) for splitting large reducers.

## Real-World Usage

tui-dispatch is used in production by [memtui](https://github.com/dmk/memtui), a TUI for Redis, Memcached, and etcd.
